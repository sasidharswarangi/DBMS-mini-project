...
class PortsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create port" do
    assert_difference('Port.count') do
      post ports_url, params: { port: { address: @port.address, license: @port.license, name: @port.name, phone_number: @port.phone_number } }
    end
  ...
  test "should update port" do
    patch port_url(@port), params: { port: { address: @port.address, license: @port.license, name: @port.name, phone_number: @port.phone_number } }
